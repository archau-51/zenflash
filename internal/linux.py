    ...
    if os.path.isdir("./platform-tools") == False:
        try:
            ...
            if down == "n":
                ...
                print(
                    Fore.GREEN + "Downloading platform-tools-linux.zip" + Fore.RESET
                )
                try:
                    file = requests.get(
                    ...
                    raise SystemExit(1)
                open("platform-tools-linux.zip", "wb").write(file.content)
                with zipfile.ZipFile("./platform-tools-linux.zip", "r") as zip_ref:
                    zip_ref.extractall(".")
                    subprocess.run(["chmod", "+x", "./platform-tools/adb"])
                    subprocess.run(["chmod", "+x", "./platform-tools/fastboot"])
                os.remove("platform-tools-linux.zip")
